...
onMounted(async () => {
  try {
    ...
    if (mapContainer.value) {
      // Initialize map
      map.value = L.map(mapContainer.value).setView([40, -95], 4) // Default to US center
      ...
      // If we have stations, fit bounds to show all
      if (stationsStore.stations.length > 0) {
        const bounds = L.latLngBounds(stationsStore.stations.map(station => 
...
function addMarkersToMap() {
  ...
  stationsStore.stations.forEach(station => {
    ...
    // Create marker
    const marker = L.marker([station.location.latitude, station.location.longitude], { icon })
      .addTo(map.value!)
...
<template>
  <!-- Changed to inline style to guarantee height -->
  <div style="height: calc(100vh - 64px);">
    ...
    <div v-else class="relative h-full">
      <!-- Map container with explicit fixed height -->
      <div ref="mapContainer" style="height: 400px;" class="z-0"></div>
      ...
      >
        ...
        <div class="mt-2">
          <span 
            :class="selectedStation.status === 'active' ? 'status-active' : 'status-inactive'"
          >
